...
public class _02_EzenComputerMainAnwser {
...
//				su.courseOut();
				...
				System.out.println(searchBranch+"지점의 학생정보 >");
//				for(int i=0; i<std.length; i++) {
//					if(std[i].getBranch().equals(searchBranch)) {
//						std[i].printInfo();
//					}
//				}
				// 향상된 for문은 완성된 배열에서만 사용 가능 : 검색 기능의 역할 (setter가 반영이 안 됨)
				for(_01_EzenComputerAnwser tmp : std) {
...
//				}
				for(_01_EzenComputerAnwser tmp : std) {
					for(int i=0; i<tmp.getCnt(); i++) {
						if(searchCourse.equals(tmp.getCourse()[i])) {
							tmp.printInfo();
							tmp.printCourse();
						}
					}
